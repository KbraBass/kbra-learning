...
const BasicMathModule = {
    ...
    renderScoreDisplay: function() {
        ...
        const modulePoints = Storage.getModulePoints(this.config.moduleId);
        const moduleLevel = Storage.getModuleLevel(this.config.moduleId);
        const levelNames = {
        ...
        if (scoreContainer) {
            scoreContainer.innerHTML = `
                <div class="score-container">
                    <span class="score-display">Puntos Globales: <span id="score">${totalPoints}</span></span>
                    <span class="score-display">Puntos Módulo: <span id="module-score">${modulePoints}</span></span>
                    <span class="score-display">Nivel Módulo: <span id="level">${levelNames[moduleLevel]}</span></span>
                </div>
            `;
        }
    },
    ...
    showMultiplicationTable : function() {
        const number = parseInt(document.getElementById('carry1').value);
        const resultDiv = document.getElementById('carry-result');
        const explanationDiv = document.getElementById('carry-explanation');

        if (isNaN(number) || number < 1 || number > 12) {
            resultDiv.textContent = '¡Por favor, introduce un número entre 1 y 12!';
            resultDiv.style.background = 'rgba(255, 107, 107, 0.3)';
            explanationDiv.innerHTML = '<h3>📝 Tabla de multiplicar:</h3><p>Introduce un número válido para ver su tabla.</p>';
            return;
        }

        let tableHTML = `
            <div style="width: 100%; display: block;">
                <h3 style="color: white; text-align: center; margin-bottom: 1rem;">Tabla de Multiplicar del ${number}</h3>
                <table class="multiplication-table">
                    ...
                    <tbody>
        `;
        for (let i = 1; i <= 12; i++) { // Generate table up to 12
            tableHTML += `
                    <tr>
                        <td>${number}</td>
                        <td>×</td>
        ...
        resultDiv.style.background = 'rgba(78, 205, 196, 0.3)';
        explanationDiv.innerHTML = `<h3>📝 ¡Aquí tienes la tabla del ${number}!</h3><p>La multiplicación es como sumar el mismo número varias veces. Por ejemplo, ${number} × 3 es igual a ${number} + ${number} + ${number}.</p>`;
    },

    showDivisionTable: function() {
        const number = parseInt(document.getElementById('division-table-number').value);
        const resultDiv = document.getElementById('division-table-result');
        const explanationDiv = document.getElementById('division-table-explanation');

        if (isNaN(number) || number < 1 || number > 12) {
            resultDiv.textContent = '¡Por favor, introduce un número entre 1 y 12!';
            resultDiv.style.background = 'rgba(255, 107, 107, 0.3)';
            explanationDiv.innerHTML += '<p>Introduce un número válido para ver su tabla.</p>';
            return;
        }

        let tableHTML = `
            <div style="width: 100%; display: block;">
                <h3 style="color: white; text-align: center; margin-bottom: 1rem;">Tabla de División por ${number}</h3>
                <table class="multiplication-table">
                    <thead>
                        <tr>
                            <th>Dividendo</th>
        ...
        resultDiv.innerHTML = tableHTML;
        resultDiv.style.background = 'rgba(78, 205, 196, 0.3)';
        explanationDiv.innerHTML = `<h3>📝 Así funciona la tabla de división:</h3><p>Por ejemplo, 36 ÷ 4 = 9 porque 4 × 9 = 36. Cada fila muestra cómo la división y la multiplicación están conectadas.</p>`;
    },
    ...
    getVisualOperationSymbol: function() {
        switch (this.config.operation) {
            case '+': return '➕';
            case '-': return '➖';
            case '*': return '✖️';
            case '/': return '➗';
            default: return this.config.operation;
        }
    },
    ...
    generateQuestion: function() {
        ...
        if (this.config.operation === '+') {
            ...
            if (moduleLevel === 'beginner') {
                ...
                num1 = num2 * (Math.floor(Math.random() * 5) + 1); // Dividend: divisor × (1-5)
            } else if (moduleLevel === 'intermediate') {
                num2 = Math.floor(Math.random() * 8) + 2; // Divisor (2-9)
                num1 = num2 * (Math.floor(Math.random() * 8) + 1); // Dividend: divisor × (1-8)
            } else {
                num2 = Math.floor(Math.random() * 12) + 1; // Divisor (1-12)
                num1 = num2 * (Math.floor(Math.random() * 12) + 1); // Dividend: divisor × (1-12)
            }
        }
        ...
        document.getElementById('practice-question').textContent = `¿Cuánto es ${num1} ${this.getVisualOperationSymbol()} ${num2}?`;
    ...
    checkAnswer: function() {
        ...
        if (userAnswer === this.currentQuestion.answer) {
            ...
            result.innerHTML = `<div class="text-dark">¡Correcto! 🎉<br>${this.currentQuestion.num1} ${this.getVisualOperationSymbol()} ${this.currentQuestion.num2} = ${this.currentQuestion.answer}<br><small>+10 puntos (Total: ${Storage.getTotalPoints()})</small></div>`;
            result.style.background = 'rgba(25, 215, 25, 0.38)';
        } else {
            Gamification.subtractPoints(15, this.config.moduleId);
            result.innerHTML = `<div class="text-dark">Incorrecto 😔<br>${this.currentQuestion.num1} ${this.getVisualOperationSymbol()} ${this.currentQuestion.num2} = ${this.currentQuestion.answer}<br>Tu respuesta: ${userAnswer}<br><small>-15 puntos (Total: ${Storage.getTotalPoints()})</small></div>`;
    ...
    startFinalExam: function() {
        ...
        // Generate 20 questions: 5 beginner, 5 intermediate, 10 advanced
        ...
        for (const { level, count } of questionCounts) {
            for (let i = 0; i < count; i++) {
                ...
                this.examQuestions.push({ num1, num2, answer: this.performOperation(num1, num2), level });
            }
        }
    ...
    showExamQuestion: function() {
        ...
        document.getElementById('exam-question').textContent = `¿Cuánto es ${q.num1} ${this.getVisualOperationSymbol()} ${q.num2}?`;
