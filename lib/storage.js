...
const Storage = (function() {
     ...
     * Get module points
     */
    function getModulePoints(moduleId) {
        const data = getUserProgress();
        return data.modules[moduleId]?.modulePoints || 0;
    }
    ...
    function getModuleLevel(moduleId) {
        const data = getUserProgress();
        return data.modules[moduleId]?.moduleLevel || 'beginner';
    }

    /**
     * Update module points and level
     */
    function updateModulePoints(moduleId, points) {
        const data = getUserProgress();
        ...
        data.modules[moduleId].modulePoints = Math.max(0, points);
        
        // Update module level based on points
        if (data.modules[moduleId].modulePoints >= 100) {
            data.modules[moduleId].moduleLevel = 'advanced';
        } else if (data.modules[moduleId].modulePoints >= 50) {
            data.modules[moduleId].moduleLevel = 'intermediate';
        } else {
            data.modules[moduleId].moduleLevel = 'beginner';
        }
