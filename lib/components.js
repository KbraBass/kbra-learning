...
const Components = (function() {
     ...
     */
    function createTabs(tabs, onTabClickFunctionName) {
        let html = '<div class="tabs">';
        
        tabs.forEach((tab, index) => {
            const activeClass = index === 0 ? 'active' : '';
            html += `
                <button class="tab-button ${activeClass}" onclick="${onTabClickFunctionName}('${tab.id}')">
                    ${tab.label}
                </button>
            `;
        });
    ...
    /**
     * Create addition visualization with two different colored groups
     */
    function createAdditionBlocks(num1, num2, maxDisplay = 40) {
        const total = num1 + num2;
        
        if (total > maxDisplay) {
            return `<p class="number-too-large">Número demasiado grande para visualizar (${total})</p>`;
        }

        let html = '<div class="number-group" style="margin: 2rem auto; max-width: 800px;">';
        
        // First group with green circles
        for (let i = 0; i < num1; i++) {
            html += `<div class="item-block">🟢</div>`;
        }
        
        // Second group with blue circles
    ...
    /**
     * Create achievement card for full list display
     */
    function createAchievementCard(achievement) {
        ...
        return `
            <div class="achievement-card ${unlocked ? 'unlocked' : 'locked'}">
                ...
                ${unlocked && achievement.unlockedDate ? `<div class="achievement-date">Desbloqueado: ${new Date(achievement.unlockedDate).toLocaleDateString()}</div>` : ''}
            </div>
        `;
    }
