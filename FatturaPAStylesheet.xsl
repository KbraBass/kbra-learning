...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  <!--
    ...
    Version: 1.3 (compact layout + print CSS + installments table + better spacing)
    Date: 2026-02-10

    Notes:
    - Pure XSLT 1.0 (works in Firefox XSLT engine)
    - Uses local-name() for namespace resilience
    - Targets FatturaPA 1.2.3 (Ordinary)
  -->
    ...
    <xsl:choose>
      ...
      <xsl:when test="$type='Natura'">
        <xsl:choose>
          ...
          <xsl:when test="starts-with($code,'N2')">N2 — Non soggette</xsl:when>
          <xsl:when test="starts-with($code,'N3')">N3 — Non imponibili</xsl:when>
          <xsl:when test="$code='N4'">N4 — Esenti</xsl:when>
          <xsl:when test="$code='N5'">N5 — Regime del margine</xsl:when>
          <xsl:when test="starts-with($code,'N6')">N6 — Inversione contabile</xsl:when>
  ...
  <xsl:template match="/">
    <html lang="it">
      <head>
        ...
        <style>
          /* Color palette */
          :root{--orange:#d64000;--green:#123015;--gold:#e9b045;--graphite:#212223;--light:#e3e3e3;--white:#ffffff}

          /* Base and spacing tuned for density */
          html,body{margin:0}
          body{font-family:'Segoe UI',Arial,sans-serif;background:var(--light);color:var(--graphite);padding:0 0 12mm 0;font-size:14px;line-height:1.35}
          header{background:linear-gradient(90deg,var(--orange) 60%, var(--green) 100%);color:var(--white);padding:10px 14px 8px 14px;border-bottom:3px solid var(--gold)}
          h1{margin:0 0 2px 0;font-size:1.6rem;letter-spacing:.5px}
          .meta{opacity:.95;font-size:.9rem}
          main{padding:8mm 10mm}

          /* Cards compacted */
          .card{background:var(--white);border-left:4px solid var(--orange);padding:8px 10px;margin:8px 0;box-shadow:0 1px 4px #0001}
          .kv-row{display:grid;grid-template-columns:170px 1fr;gap:6px;margin:2px 0;align-items:start}
          .kv-label{font-weight:600;color:var(--green)}
          .kv-value{color:var(--graphite)}

          /* Grids */
          .grid-header{display:grid;grid-template-columns:repeat(auto-fit,minmax(360px,1fr));gap:8px}

          /* Tables condensed */
          table{border-collapse:collapse;width:100%;background:var(--white);margin:6px 0;box-shadow:0 1px 4px #0001}
          th{background:var(--green);color:var(--white);padding:6px 6px;font-weight:600;border:1px solid var(--green)}
          td{border:1px solid var(--light);padding:5px 6px;vertical-align:top}
          tr:nth-child(even){background:#f7f7f7}

          .btn{display:inline-block;background:var(--orange);color:#fff!important;padding:.28rem .55rem;border-radius:5px;border:1px solid #a33000;text-decoration:none;font-weight:600}
          ...
          @media print{
            ...
            header{padding:6mm 0 2mm 0;border-bottom:2px solid var(--gold)}
            main{padding:6mm 6mm}
            .card{box-shadow:none;margin:6px 0;padding:6px 8px;border-left:3px solid var(--orange)}
            .kv-row{grid-template-columns:140px 1fr;gap:4px;margin:1px 0}
            .grid-header{grid-template-columns:repeat(2, 1fr);gap:6px}
            th{padding:5px 5px}
            td{padding:4px 5px}
          }
        </style>
  ...
  <xsl:template match="*[local-name()='FatturaElettronica']">
    <div class="grid-header avoid-break">
      <div class="card kv"><h2>Cedente/Prestatore</h2><xsl:apply-templates select="*[local-name()='FatturaElettronicaHeader']/*[local-name()='CedentePrestatore']" mode="kv"/></div>
      <div class="card kv"><h2>Cessionario/Committente</h2><xsl:apply-templates select="*[local-name()='FatturaElettronicaHeader']/*[local-name()='CessionarioCommittente']" mode="kv"/></div>
      <div class="card kv"><h2>Dati Trasmissione</h2><xsl:apply-templates select="*[local-name()='FatturaElettronicaHeader']/*[local-name()='DatiTrasmissione']" mode="kv"/></div>
      <div class="card kv"><h2>Terzo Intermediario / Soggetto Emittente</h2><xsl:apply-templates select="*[local-name()='FatturaElettronicaHeader']/*[local-name()='TerzoIntermediarioOSoggettoEmittente']" mode="kv"/></div>
    </div>

    <xsl:apply-templates select="*[local-name()='FatturaElettronicaBody']"/>
  </xsl:template>

  <!-- ===== Header sections (KV mode) ===== -->
  ...
  <xsl:template name="render-beni-servizi">
    ...
    <table class="avoid-break">
      <thead>
        <tr>
          <th>#</th><th>Descrizione</th><th>Q.tà</th><th>UM</th><th>Prezzo</th><th>Sconto/Magg.</th><th>Aliq.</th><th>Natura</th><th>Totale</th>
        </tr>
      </thead>
      <tbody>
              ...
              <td colspan="8">
                <ul>
                  <xsl:for-each select="*[local-name()='AltriDatiGestionali']">
                    <li><strong><xsl:value-of select="*[local-name()='TipoDato']"/></strong>: <xsl:value-of select="*[local-name()='RiferimentoTesto'] | *[local-name()='RiferimentoNumero'] | *[local-name()='RiferimentoData']"/></li>
                  </xsl:for-each>
                </ul>
              </td>
            </tr>
  ...
  <!-- Pagamenti: render by DatiPagamento, with clear installments table -->
  <xsl:template name="render-pagamento">
    <xsl:for-each select="*[local-name()='DatiPagamento']">
      <xsl:variable name="cond" select="*[local-name()='CondizioniPagamento']"/>
      <div class="kv">
        <div class="kv-row"><div class="kv-label">Condizioni</div><div class="kv-value"><xsl:value-of select="$cond"/></div></div>
      </div>
      <table class="avoid-break">
        <thead>
          <tr>
        ...
        <tbody>
          <xsl:for-each select="*[local-name()='DettaglioPagamento']">
            <tr>
              <td><xsl:value-of select="position()"/></td>
              <td><xsl:value-of select="*[local-name()='DataScadenzaPagamento']"/></td>
              <td><xsl:value-of select="*[local-name()='ImportoPagamento']"/></td>
              <td>
              ...
              <td><xsl:value-of select="*[local-name()='IBAN']"/></td>
              <td><xsl:value-of select="*[local-name()='BIC']"/></td>
              <td><xsl:value-of select="*[local-name()='IstitutoFinanziario']"/></td>
              <td><xsl:value-of select="*[local-name()='CodicePagamento']"/></td>
            </tr>
          </xsl:for-each>
        </tbody>
      </table>
